...
data UpDown = ModePlus Int | ModeMinus Int
...
get :: UpDown -> Int
get (ModePlus count) = count
get (ModeMinus count) = count

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (ModePlus count) = ModePlus (count + 1)
tick (ModeMinus count) = ModeMinus (count - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (ModePlus count) = ModeMinus count
toggle (ModeMinus count) = ModePlus count
...
rgb :: Color -> [Double]
rgb color = case color of
  Red -> [1, 0, 0]
  Green -> [0, 1, 0]
  Blue -> [0, 0, 1]
  Mix c1 c2 -> zipWith (\a b -> (a + b) / 2) (rgb c1) (rgb c2)
...
toList Empty = []
toList (Pair k v pairs) = (k, v) : toList pairs
...
prettyPrint bin = case bin of
  End -> ""
  O bin -> prettyPrint bin ++ "0"
  I bin -> prettyPrint bin ++ "1"

fromBin :: Bin -> Int
fromBin bin = case bin of
  O bin -> 2 * fromBin bin
  I bin -> 1 + 2 * fromBin bin
  End -> 0

toBin :: Int -> Bin
toBin value = toBin' value True
  where
    toBin' value isFirst
      | value == 0 && isFirst = O End
      | value == 0 = End
      | even value = O (toBin' (value `div` 2) False)
      | otherwise = I (toBin' (value `div` 2) False)
