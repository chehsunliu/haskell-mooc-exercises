...
firstHalf s = take ((1 + length s) `div` 2) s
...
capitalize s = unwords $ map capitalizeFirst $ words s
  where
    capitalizeFirst (head : tail) = toUpper head : tail
    capitalizeFirst "" = ""
...
joinToLength l strings = [s1 ++ s2 | s1 <- strings, s2 <- strings, l == length s1 + length s2]
...
multiCompose fs = multiCompose' fs id
  where
    multiCompose' [] g = g
    multiCompose' (f : fs) g = multiCompose' fs (g . f)
...
multiApp :: ([d] -> b) -> [c -> d] -> c -> b
multiApp f gs x = f $ multiApp' gs x []
  where
    multiApp' [] _ result = result
    multiApp' (g : gs) x result = multiApp' gs x (result ++ [g x])
...
interpreter commands = f commands 0 0
  where
    f [] x y = []
    f (cmd : cmds) x y = case cmd of
      "up" -> f cmds x (y + 1)
      "down" -> f cmds x (y - 1)
      "left" -> f cmds (x - 1) y
      "right" -> f cmds (x + 1) y
      "printX" -> show x : f cmds x y
      _ -> show y : f cmds x y