...
member x (Set xs) = foldr (f x) False xs
  where
    f target x result = target == x || result
...
data State = Start | Error | Finished | EggsDone | EggsFlourDone | EggsSugarDone | BeforeMix | BeforeBake
  deriving (Eq, Show)

step :: State -> Event -> State
step Start AddEggs = EggsDone
step EggsDone AddFlour = EggsFlourDone
step EggsDone AddSugar = EggsSugarDone
step EggsFlourDone AddSugar = BeforeMix
step EggsSugarDone AddFlour = BeforeMix
step BeforeMix Mix = BeforeBake
step BeforeBake Bake = Finished
...
instance Ord a => Semigroup (Set a) where
  (<>) (Set []) (Set b) = Set b
  (<>) (Set (a : as)) (Set b) = (<>) (Set as) (add a (Set b))
...
passwordAllowed s1 (ContainsSome s2) = check s1
  where
    check [] = False
    check (c1 : s1) = check s1 || foldr (\c2 result -> c1 == c2 || result) False s2
passwordAllowed s1 (DoesNotContain s2) = not (passwordAllowed s1 (ContainsSome s2))
...
evaluate (Operation op a1 a2) = case op of
  "+" -> evaluate a1 + evaluate a2
  _ -> evaluate a1 * evaluate a2
