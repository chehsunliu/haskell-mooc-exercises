...
treeMax Empty = 0
treeMax (Node value l r) = maximum [value, treeMax l, treeMax r]
...
allValues condition (Node value l r) = condition value && allValues condition l && allValues condition r
...
cull targetValue (Node value l r)
  | targetValue == value = Empty
  | otherwise = Node value (cull targetValue l) (cull targetValue r)
cull _ Empty = Empty
...
isOrdered (Node value l r) = allValues (< value) l && allValues (> value) r && isOrdered l && isOrdered r
...
walk [] (Node value l r) = Just value
walk (step : steps) (Node _ l r) = case step of
  StepL -> walk steps l
  StepR -> walk steps r
...
set _ _ Empty = Empty
set [] newValue (Node _ l r) = Node newValue l r
set (step : steps) newValue (Node value l r) = case step of
  StepL -> Node value (set steps newValue l) r
  StepR -> Node value l (set steps newValue r)
...
search target Empty = Nothing
search target (Node value l r)
  | target == value = Just []
